...
module.exports.search = (source, request, fields, where, orderBy, start, limit, callback) => {
    db.getConnection((err, connection) => {
        if (err) {
            return callback(err);
        }

        let limitQuery = ' Limit ' + limit + ' OFFSET ' + start;

        let sql = 'SELECT SQL_CALC_FOUND_ROWS ' + fields.join(', ') + ' FROM ' + source + (where === '' ? '' : ' WHERE ' + where) + ' ORDER BY ' + orderBy + limitQuery;
