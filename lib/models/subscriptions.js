...
module.exports.emailExsitInList = (id, email, callback) => {
    if (!id) {
        return callback(new Error(_('Missing ID')));
    }
    if (!email) {
        return callback(new Error(_('Missing Subscription email address')));
    }
    ...
    db.getConnection((err, connection) => {
        ...
        connection.query('SELECT id FROM `subscription__' + id + '` WHERE email=?', [email], (err, rows) => {
            connection.release();
            if (err) {
                return callback(err);
            }
