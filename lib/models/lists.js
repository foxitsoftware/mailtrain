...
module.exports.quicklist = callback => {
    db.getConnection((err, connection) => {
        ...
        connection.query('SELECT id, cid, name, subscribers, created FROM lists ORDER BY name LIMIT 1000', (err, rows) => {
            if (err) {
                connection.release();
                return callback(err);
            }
...
module.exports.getLists = (request, where, start, limit, callback) => {
    tableHelpers.search('lists', request,
        ['id', 'cid', 'name', 'subscribers', 'created'],
        where, 'created DESC', start, limit, callback);
};
...
module.exports.getById = (cid, callback) => {
    resolveId(cid, (err, id) => {
        if (err) {
            return callback(err);
        }
        if (!id) {
...
module.exports.getListBaseDetailById = (id, callback) => {
    if (!id) {
    ...
    db.getConnection((err, connection) => {
        ...
        connection.query('SELECT id, cid, name, description, subscribers, created FROM lists WHERE id=?', [id], (err, rows) => {
            connection.release();
            if (err) {
                return callback(err);
            }
            return callback(null, rows && rows[0] && rows[0] || false);
        });
    });
}
...
module.exports.listById = (request, list_id, where, orderBy, start, limit, callback) => {
    return tableHelpers.search('subscription__' + list_id, request,
        ['id', 'cid', 'email', 'first_name', 'last_name', 'tz', 'status', 'is_test', 'opt_in_country', 'created'],
        where, orderBy, start, limit, callback);
};
...
function resolveId(cid, callback) {
    cid = (cid || '').toString().trim();
    if (!cid) {
        return callback(new Error(_('Missing List ID')));
    }
    ...
    db.getConnection((err, connection) => {
        ...
        connection.query('SELECT id FROM lists WHERE id=?', [cid], (err, rows) => {
            connection.release();
            if (err) {
                return callback(err);
            }
